...
// import {reqSubmitOrder} from '@/api'//在main.js中引入全部API，挂载到vue原型对象不需要每个组件分别引用 
  export default {
    name: 'Trade',
    data(){
      return{
    ...
    mounted(){;
    this.$store.dispatch('getUserAddress');   //获取用户地址信息
    this.$store.dispatch('getOrderInfo');     //获取用户商品信息
  },
  computed:{
    ...mapState({
  ...
  methods:{
    ...
    async submitOrder(){
      ...
      let data = {
        ...
        deliveryAddress : this.userDefaultAddress.fullAddress,//收集人地址
        paymentWay : "ONLINE",//支付方式
        orderComment : this.msg,//留言信息
        orderDetailList : this.orderInfo.detailArrayList,//商品清单
      }
      let result = await this.$API.reqSubmitOrder(tradeNo,data);//API中需要带参，tradeNo data
      console.log("订单提交请求："+result)
      // 提交订单成功
      if(result.code==200){
